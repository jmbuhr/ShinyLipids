function(input, output, session) {
    ...
    PCA.results <- reactive({
        ...
        pca.inputs <- list()
        pca_df <- SubPlotData()
        pca_df <- pca_df[c('sample','sample_replicate','xval','standardizedSum')] %>%
    ...
    plotInput <- reactive({
        ...
        plt_main <- preparePlots(SubPlotData(), checkGroup = input$checkGroup,
    ...
    plotPCA <- reactive({
        ...
        print("PCA started")
        A <- SubPlotData()[c('sample_replicate','xval','standardizedSum')] %>%
    ...
    output$saveplot <- downloadHandler(
        ...
        content = function(file) {
            ggsave(file, plotInput(), device = "pdf", width = 20, height = 10)
        }
    )
    ...
    output$save <- downloadHandler(
        ...
        content = function(file) {
            ggsave(file, plotInput(), device = "pdf", width = input$width, height = input$height)
        }
    )
