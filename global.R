...
devium.pca.calculate<-function(pca.inputs,args.list=TRUE,return="list", plot=TRUE)
{
    ...
    if(return=="list"){
        return(list(pca.scores = scores, pca.loadings =  loadings,pca.eigenvalues = eigenvalues, pca.diagnostics = diagnostics, pca.groups = tmp$pca.groups, pca.model=pca.results))}
...
binbase.format.output<-function(data,col.meta,row.meta){
    ...
    space<-dim(tmp)[1]-1
    tmp2<-rbind("",matrix("",space-dim(data)[1],ncol(row.meta)),as.matrix(row.meta))
